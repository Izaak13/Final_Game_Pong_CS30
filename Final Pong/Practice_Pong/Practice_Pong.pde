...
if ((myBall.x < (2*myBall.diameter) || myBall.x > ( width-(2*myBall.diameter)) || movedBall.x < (2*movedBall.diameter) || movedBall.x > ( width-(2*movedBall.diameter)))) {
  ...
  if (opponentSelect == 0) textDraw(black, CENTER, TOP, font, "Player 1", width/15, height/60, width/6, height/15);
  else textDraw(black, CENTER, TOP, font, "Player", width/15, height/60, width/6, height/15);
  if (opponentSelect == 0) textDraw(black, CENTER, TOP, font, "Player 2", width*14/15-width/6, height/60, width/6, height/15);
  else textDraw(black, CENTER, TOP, font, "AI", width*14/15-width/6, height/60, width/6, height/15);
  textDraw(black, CENTER, TOP, font, "RESET", width*4.5/10, height*59/60-height/15, width/10, height/15);
  if (opponentSelect == 0) textDraw(black, CENTER, TOP, font, "TWO PLAYER", width/10, height*59/60-height/15, width/6, height/15);
  if (opponentSelect == 1) textDraw(black, CENTER, TOP, font, "EASY", width/10, height*59/60-height/15, width/6, height/15);
  if (opponentSelect == 2) textDraw(black, CENTER, TOP, font, "MEDIUM", width/10, height*59/60-height/15, width/6, height/15);
  if (opponentSelect == 3) textDraw(black, CENTER, TOP, font, "HARD", width/10, height*59/60-height/15, width/6, height/15);
  //
  yourPaddle.AI();
  //
  //Start screen
  if (start==false) {
    textDraw(green, CENTER, TOP, boldFont, "WELCOME   TO PONG!!", 0, height/5, width, height/10);
    textDraw(white, CENTER, TOP, boldFont, "W = up, S = down", -width/4, height/2.1, width, height/10);
    textDraw(white, CENTER, TOP, boldFont, "I = up, K = down", width/4, height/2.1, width, height/10);
    textDraw(green, CENTER, TOP, boldFont, "press a key  to continue", 0, height*3/4, width, height/10);
  }
  //
} //end draw
//
...
void mousePressed() {
  ...
  if (mouseX >= (width/10) && mouseX <= (width/10+width/6) && mouseY >= (height*59/60-height/15) && mouseY <= height*59/60) { //Opponent selector
    if (opponentSelect < 3) {
      opponentSelect++;
      println(opponentSelect);
    }
    else
    {
      opponentSelect = 0;
