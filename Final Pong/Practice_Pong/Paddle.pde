...
class Paddle {
  ...
  void AI() {
    if (opponentSelect == 1) {
      if (myBall.x > width/2) {
        if (myBall.y > this.paddleY+paddleHeight/2) movePaddleDown();
        if (myBall.y < this.paddleY+paddleHeight/2) movePaddleUp();
      }
    }
    if (opponentSelect == 2) {
      if (myBall.x > width/2) {
    ...
    if (opponentSelect == 3) {
      if (myBall.x > width/3 && myBall.xSpeed>0) {
        if (myBall.ySpeed > 0 || (myBall.y > height*3/4 && myBall.x > width*3/4)) {
          if (myBall.y > this.paddleY ) movePaddleDownFast();
        }
        if (myBall.ySpeed < 0 || (myBall.y < height*1/4 && myBall.x > width*3/4)) {
          if (myBall.y < this.paddleY+paddleHeight) movePaddleUpFast();
        }
      }
    }
  }//end AI
  ...
  //
  void movePaddleDownFast() {
    paddleY += paddleMove*1.15;
    if ( paddleY > tableY+tableHeight-paddleHeight ) paddleY = tableY+tableHeight-paddleHeight;
    //this.down=false; //ERROR: boolean reset means only one paddle at a time, no gaming
  } //end move paddleDown
  //
  void movePaddleUpFast() {
    paddleY -= paddleMove*1.15;
    if ( paddleY < tableY ) paddleY = tableY; //error catch
    //this.up=false; //ERROR: boolean reset means only one paddle at a time, no gaming
  } //end move paddleUp
